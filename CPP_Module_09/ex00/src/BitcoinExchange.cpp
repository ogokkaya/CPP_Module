...
bool Date::isValid() const {
    ...
    // 30 çeken aylar kontrolü (Şubat bloğunun dışında olmalı!)
...
// DÜZELTME: ptr_fun içine artık my_isspace gönderiliyor
...
BitcoinExchange &BitcoinExchange::operator=(const BitcoinExchange &other) {
    if (this != &other) {
        _data = other._data; // DÜZELTME: _date yerine _data
    }
    return (*this);
}
...
BitcoinExchange::BitcoinExchange() {
    std::ifstream file("data.csv"); // DÜZELTME: Date.csv değil data.csv
    
    if (!file.is_open())
        throw std::runtime_error("Error: could not open database file.");
    
    std::string line;
    std::getline(file, line); // Header atla
    ...
    while (std::getline(file, line)) {
        ...
        try {
            // DÜZELTME: Değişken adı 'date' (küçük harf) yapıldı
            Date date(rawDate); 
            float rate = safeStof(rawRate);
            _data[date] = rate; // DÜZELTME: _date yerine _data
...
float BitcoinExchange::safeStof(const std::string &str) {
    ...
    if (str.c_str() == endPtr) throw std::invalid_argument("Not a number");
    if (*endPtr != '\0') throw std::invalid_argument("Invalid characters in number");
...
void BitcoinExchange::run(const std::string &fileName) {
    ...
    while (std::getline(file, line)) {
        ...
        // DÜZELTME: İsim Çakışması Önledi. Date -> date (küçük harf)
        Date date(rawDate); 
        ...
        // DÜZELTME: lower_bound artık 'date' (nesne) alıyor, 'Date' (tür) değil.
        // Ayrıca _date yerine _data kullanıldı.
